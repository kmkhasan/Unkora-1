...
function PriceSlider({ min, max, onChange }: { min: number; max: number; onChange: (min: number, max: number) => void }) {
  ...
  useEffect(() => {
    ...
    typeof window !== "undefined" && window.addEventListener('mouseup', up);
    return () => { typeof window !== "undefined" && window.removeEventListener('mousemove', onMouseMove); typeof window !== "undefined" && window.removeEventListener('mouseup', up); };
  }, [onMouseMove]);
